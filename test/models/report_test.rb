...
class ReportTest < ActiveSupport::TestCase
  ...
  test 'save_mentions should save new mentions scanned from report content without own id' do
    ...
    @report.content = "I am http://localhost:3000/reports/#{@report.id}. http://localhost:3000/reports/2 is good. http://localhost:3000/reports/3 is normal."

    @report.send(:save_mentions)

    assert_equal([2, 3], @report.mentioning_report_ids)
  end
end
